...
const primeraImagenCategoria = (cat) => {
  for (const pc of cat.Producto_Categoria ?? []) {
    const img = pc?.Producto?.Imagen?.[0]?.imagen;
    if (img) return img;
  }
  return null;
};

function SearchBar({ resultados = [], marcas = [], categorias = [], textoBusqueda = "", cargando = false }) {
  if (!textoBusqueda.trim()) return null;

  const total = resultados.length + marcas.length + categorias.length;
  ...
  return (
    <section className="search-results">
      <div className="search-results__header">
        ...
        <span>{cargando ? "" : `${total} items`}</span>
      </div>
        ...
        <>
          {categorias.length > 0 && (
            <div className="search-results__seccion">
              <p className="search-results__seccion-titulo">Categorías</p>
              <div className="search-results__grid">
                {categorias.slice(0, 4).map((cat) => {
                  const imagen = primeraImagenCategoria(cat);
                  return (
                    <article key={`cat-${cat.id_categoria}`} className="search-results__card">
                      <Link to={`/categoria/${slugify(cat.nombre)}`} className="search-results__link">
                        {imagen ? (
                          <img src={imagen} alt={cat.nombre} />
                        ) : (
                          <div className="search-results__placeholder">{cat.nombre.slice(0, 1)}</div>
                        )}
